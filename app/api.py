...
class PageModelViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        """ Переопределение страндартного метода. Установка сериализатора в зависимости от типа API-запроса. """
        if self.action == 'retrieve':
            return PageDetailSerializer
        return PageSerializer

    def retrieve(self, request, *args, **kwargs):
        """ Переопределение страндартного метода. Вызов метода модели при обработке API-запроса. """
